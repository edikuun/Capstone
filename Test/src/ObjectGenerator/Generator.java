    ...
    public void update() {
        ...
        while (Math.abs(platforms.getLast().getHitBounds().getY() - player.getBounds().getY()) < generationBorder) {
            ...
            if (currentBiome == null || currentBiome instanceof NormalLevel) {
                ...
                for (Level b : biomes) {
                    if (b.shouldEnter(player.getScore())) {
                        currentBiome = b.createGeneratorInstance(player.getScore());
                        System.out.println("Entered " + currentBiome.toString());
                        break;
                    }
                }
                if (currentBiome == null) {
                    currentBiome = baseBiome.createGeneratorInstance(player.getScore());
                }
            }

            currentBiome.generate(maximalJumpHeight, player.getScore());
