...
public class HighscoreState extends BasicGameState {
    ...
    private void calcFillingRect(GameContainer container) {
        ...
        int minY = y / 2;
        int maxY = y + highscore.getNumberOfEntries() * font.getLineHeight();
        int minX = 0;
        int maxX = 0;
        minX = (container.getWidth() - fontHeader.getWidth(headerText)) / 2;
        maxX = minX + fontHeader.getWidth(headerText);
        List<HighscoreEntry> entries = highscore.getEntries();
        for (int i = 0; i < entries.size(); i++) {
            String s = (i + 1) + ". " + entries.get(i).toString();
            if (i == highlight) {
